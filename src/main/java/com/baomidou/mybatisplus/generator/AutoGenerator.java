...
public class AutoGenerator {
	...
	protected com.baomidou.mybatisplus.generator.ConfigGenerator config;

	public com.baomidou.mybatisplus.generator.ConfigGenerator getConfig() {
		return config;
	}

	public void setConfig(com.baomidou.mybatisplus.generator.ConfigGenerator config) {
		this.config = config;
	}
	...
	public AutoGenerator(com.baomidou.mybatisplus.generator.ConfigGenerator config) {
		this.config = config;
	}
	...
	public static void run(ConfigGenerator config) {
		 ...
		 * 开启生成映射关系
		 */
		new com.baomidou.mybatisplus.generator.AutoGenerator(config).generate();

		System.out.println(" the files is already generated in:"+ config.getSaveDir() + " successful! ");
	...
	public void generate() {
		...
		try {
			...
			for (String table : tables) {
				...
				String beanName = getBeanName(table,config.getEntityName(), config.isDbPrefix());
	...
	protected List<String> getTables(Connection conn) throws SQLException {
		...
		String[] tableNames = new String[]{config.getTableName()};
	 ...
	 * @return beanName
	 */
	protected String getBeanName(String table,String entityName, boolean includePrefix) {
		StringBuilder sb = new StringBuilder();
		if (entityName != null) {
			sb.append(entityName.substring(0, 1).toUpperCase()).append(entityName.substring(1));
		} else {
			if (table.contains("_")) {
	...
	protected void buildEntityBean(List<String> columns, List<String> types, List<String> comments, String tableComment,
		...
		if (config.getIdType() != IdType.ID_WORKER) {
			bw.write("import com.mmh.framework.annotations.IdType;");
			bw.newLine();
		}
		bw.newLine();
		/*
		...
		bw.write("import com.mmh.framework.annotations.TableField;");
		bw.newLine();
		if (null == config.getConfigBaseEntity()) {
			bw.write("import com.mmh.framework.annotations.TableId;");
			bw.newLine();
		}
		if (table.contains("_") || null != config.getEntityName()) {
			bw.write("import com.mmh.framework.annotations.TableName;");
			bw.newLine();
		}
		bw = buildClassComment(bw, tableComment);
		bw.newLine();

		if (null != config.getEntityName()) {
	...
	protected void buildMapper(String beanName, String mapperName) throws IOException {
		...
		if (config.getConfigIdType() == ConfigIdType.STRING) {
			bw.write("import com.mmh.framework.service.mapper.CommonMapper;");
		} else {
			bw.write("import com.mmh.framework.service.mapper.AutoMapper;");
		}
		bw.newLine();
	...
	protected void buildService(String beanName, String serviceName) throws IOException {
		...
		bw = buildClassComment(bw, beanName + " 数据服务层接口");
		bw.newLine();
	...
	protected void buildServiceImpl(String beanName, String serviceImplName, String serviceName, String mapperName)
		...
		bw = buildClassComment(bw, beanName + " 服务层接口实现类");
		bw.newLine();
		bw.write("@Service");
		bw.newLine();
